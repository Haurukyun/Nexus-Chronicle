                ...
                {trash.map((e: WorldEntity) => (
                    <div key={e.id} className={`flex items-center justify-between p-4 rounded-xl border ${isWikiMode ? 'bg-white border-[#d4c8af]' : 'bg-slate-900/40 border-slate-800'}`}>
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setWorld((p) => ({ ...p, trash: p.trash.filter((x) => x.id !== e.id), entities: [...p.entities, e] }))} className="p-2 hover:bg-green-500/10 text-green-500 transition-all"><RefreshCw size={20} /></button>
                            <button onClick={() => setWorld((p) => ({ ...p, trash: p.trash.filter((x) => x.id !== e.id) }))} className="p-2 hover:bg-rose-500/10 text-rose-500 transition-all"><Trash size={20} /></button>
                        </div>
                    </div>
                ))}
            </div>
