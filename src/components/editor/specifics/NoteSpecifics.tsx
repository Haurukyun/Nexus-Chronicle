...
export const NoteSpecifics: React.FC<Props> = ({ entity, allEntities, onUpdate, onCreateNew, isWikiMode }) => {
    return (
        <EditorGroup title="Note Specifics" icon={FileText} isWikiMode={isWikiMode}>
            ...
            <div className="lg:col-span-3">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest pl-1 mb-1 block">Free-form notes</label>
                <textarea className={`w-full ${isWikiMode ? 'bg-white border-[#d4c8af]' : 'bg-slate-800/40 border-slate-700'} border rounded-xl px-4 py-3 h-32 outline-none resize-none text-sm shadow-sm`} value={entity.textNote || ''} onChange={e => onUpdate({ ...entity, textNote: e.target.value })} />
            </div>
            ...
            <SmartSelect label="Connected to Lore notes/Other notes" ids={entity.pairedConnectedNote || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedNote: ids })} onCreate={onCreateNew} />
            <SmartSelect label="Connected to Myths/Legends/Stories" ids={entity.pairedConnectedMyths || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedMyths: ids })} onCreate={onCreateNew} />
            ...
            <SmartSelect label="Connected to Locations/Geography" ids={entity.pairedConnectedLocation || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedLocation: ids })} onCreate={onCreateNew} />
            <SmartSelect label="Connected to Events" ids={entity.pairedConnectedEvents || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedEvents: ids })} onCreate={onCreateNew} />
            ...
            <SmartSelect label="Connected to Languages" ids={entity.localLanguages || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, localLanguages: ids })} onCreate={onCreateNew} />
            <SmartSelect label="Connected to Culture/Art" ids={entity.pairedConnectedCultures || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedCultures: ids })} onCreate={onCreateNew} />
            ...
            <SmartSelect label="Connected to Skills/Spells/Other" ids={entity.pairedConnectedSkills || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedSkills: ids })} onCreate={onCreateNew} />
            ...
            <SmartSelect label="Connected to Afflictions/Boons/Conditions" ids={entity.pairedConnectedConditions || []} type="character" all={allEntities} isWikiMode={isWikiMode} onChange={(ids) => onUpdate({ ...entity, pairedConnectedConditions: ids })} onCreate={onCreateNew} />
